...
十类题型固定为：事实复述、原理解释、代码走读、接口契约、故障排查、性能成本、测试验证、一致性幂等、安全可靠性、架构设计。

| 题型 | 题号范围（主归类） | 数量 |
|---|---|---:|
| 事实复述 | 1-3, 11-12, 25-26 | 7 |
| 原理解释 | 4-6, 14-17, 20-21 | 9 |
| 代码走读 | 7-10, 13, 18-19, 22-23 | 9 |
| 接口契约 | 24, 28-29, 36-37, 39-40 | 7 |
...
## 4. Level 1—5 题目覆盖矩阵（Task 3 题号预留）

以下仅给出编号、主题、考察点和事实边界；Task 3 必须沿用编号，不在此生成题库全文。

### Level 1：项目事实与主链路（12 题）

| 题号 | 预留题目主题 | 覆盖知识点 | 标签边界 |
|---:|---|---|---|
| 1 | 项目一句话定位和输入输出 | 业务、FastAPI、MySQL、Redis、Agent | 源码已确认；收益待验证 |
| 2 | sku_info 哪些字段进入 page_content | 商品加载、正文构造 | 源码已确认 |
| 3 | metadata 保存什么以及用途边界 | metadata | 源码已确认；非硬过滤 |
| 4 | chunk size/overlap 与 splitter 配置 | 切分 | 源码已确认；效果待验证 |
| 5 | Embedding 工厂如何接 OpenAI-compatible API | Embedding、Provider | 源码已确认；连通性待验证 |
| 6 | 当前向量存储是什么 | Redis Vector | 源码已确认；索引状态待验证 |
| 7 | 同步批次和文档 ID 如何生成 | 批量、MD5 ID | 源码已确认 |
| 8 | 推荐请求的 Agent 由哪些部分组成 | Agent、Prompt、Tool | 源码已确认 |
| 9 | vector_search_tool 召回多少条 | top-k | 源码已确认 k=10 |
| 10 | 推荐结构化响应有哪些层次 | Schema、Result | 源码已确认 |
| 11 | extract 当前能提取哪些字段 | 条件提取 | 源码已确认；Word描述更宽 |
| 12 | Python 服务暴露哪些核心路由 | FastAPI | 源码已确认 |

### Level 2：组件原理与代码执行（18 题）

| 题号 | 预留题目主题 | 覆盖知识点 | 标签边界 |
|---:|---|---|---|
| 13 | 从 SQLDatabaseLoader 到文档的执行顺序 | 商品加载、懒加载 | 源码已确认 |
| 14 | page_content 与 metadata 分离的 RAG 意义 | RAG 数据建模 | 原理基于源码；效果待验证 |
| 15 | 为什么 metadata 复制全部查询行 | metadata、结果展示 | 源码已确认；不等于过滤 |
| 16 | 256/25 分片如何影响召回上下文 | Chunk、Embedding | 原理讨论；无基准 |
| 17 | Embedding 工厂缓存的作用与边界 | Provider、成本 | 源码已确认；无故障转移 |
| 18 | RedisVectorStore 写入如何承接向量化 | Redis Vector、Embedding | 源码已确认；索引兼容待验证 |
| 19 | MD5 文档 ID 在内容变化时的含义 | ID、重复/旧文档 | 源码已确认；清理未实现 |
| 20 | Agent 和普通函数调用的职责区别 | Agent、Tool calling | 源码已确认；真实调用待验证 |
| 21 | k=10 与前端显示 5 的区别 | top-k、前端协作 | 源码已确认 |
| 22 | Pydantic 结构校验不能保证什么 | structured response | 源码已确认；业务回查为规划 |
| 23 | 条件提取链的 parser 如何工作 | extract、Schema | 源码已确认 |
| 24 | `/recommend` 与 `/extract` 的职责边界 | API、前端 | 源码已确认；Java映射待验证 |
| 25 | thread_id 如何传入 Agent config | 会话 | 源码已确认 |
| 26 | 两类 Embedding 与两类 LLM Provider | Provider 抽象 | 源码已确认；运行值待验证 |
| 27 | 全局异常 handler 的响应风险 | 异常、HTTP status | body 已确认；HTTP status 待验证 |
| 28 | AI 模式为何并行 recommend/extract | 前端协作、降级 | 源码已确认 |
| 29 | `/v1/search/*` 到 `/api/v1/*` 的契约问题 | Java/Gateway、DTO | Word描述/待验证 |
| 30 | 现有测试真正覆盖哪些路径 | 测试边界 | 源码已确认 |

### Level 3：场景排障与局部设计（14 题）

| 题号 | 预留题目主题 | 覆盖知识点 | 标签边界 |
|---:|---|---|---|
| 31 | 只有 Redis 向量工具时如何补业务过滤 | Agent、过滤 | 现状确认；方案为架构规划 |
| 32 | 召回 k=10 但相关性差如何定位 | top-k、召回 | 指标待验证；方案为规划 |
| 33 | 结构化响应合法但商品不存在怎么办 | Schema、业务回查 | 缺口源码已确认；回查为规划 |
| 34 | 品牌/库存诉求为何可能提取失败 | 条件 Schema、Word差异 | 源码已确认；扩展为规划 |
| 35 | `/sync` 同步阻塞请求时如何处理 | FastAPI、任务化 | 阻塞现状已确认；异步化为规划 |
| 36 | threadId/thread_id 不一致如何排查串会话 | 会话、前端契约 | 风险待验证 |
| 37 | 推荐接口不消费 extract 结果如何解释 | 前后端协作、接口 | 源码已确认；整合层待验证 |
| 38 | 模型解析错误和 Redis 超时如何统一诊断 | 异常、Provider、Redis | 捕获行为待验证 |
| 39 | Promise.allSettled 一方失败时的产品语义 | 前端、降级 | 源码已确认；协议规划 |
| 40 | Java/Feign DTO 应怎样适配两套命名 | Java 集成、契约 | Word描述/架构规划 |
| 41 | 如何把手工 async 示例变成可执行测试 | 测试 | 现状已确认；测试方案为规划 |
| 42 | 全量同步的性能瓶颈和成本来源 | 同步、Embedding、批量 | 源码已确认；数据量指标待验证 |
| 43 | 需要补哪些日志和指标定位长请求 | 观测、性能 | 架构规划 |
| 44 | 如何设计端到端联调验证链路 | Java/Gateway、FastAPI、前端 | 待验证项；架构规划 |

### Level 4：项目深挖与生产化方案（12 题）

| 题号 | 预留题目主题 | 覆盖知识点 | 标签边界 |
|---:|---|---|---|
| 45 | 设计增量、删除和版本化同步 | 一致性、幂等 | 当前缺口；架构规划 |
| 46 | 设计推荐 SKU 白名单和实时回查 | 安全、业务正确性 | 当前缺口；架构规划 |
| 47 | 错误信息如何脱敏并保持可排障 | 异常、安全、观测 | 当前泄露风险已确认；方案规划 |
| 48 | 混合检索、Rerank 是否值得引入 | Redis、Rerank | 未实现；架构规划，不能写现有 |
| 49 | 如何为同步和查询设置 timeout/retry | 性能、可靠性、Provider | 当前配置不完整；方案规划 |
| 50 | 如何建立离线查询评估集 | 测试、Recall/NDCG | 当前无评估；架构规划 |
| 51 | 同步任务如何做到幂等、锁和断点 | 一致性、任务可靠性 | 当前无实现；架构规划 |
| 52 | InMemorySaver 改造成多实例会话 | Checkpoint、会话隔离 | Redis checkpoint 仅规划 |
| 53 | 如何制定 P95、成本和召回 SLO | 性能、成本、评估 | 无实测；架构规划 |
| 54 | Agent/Tool/Schema 的安全边界如何设计 | Agent、结构化输出 | 当前只有格式校验；方案规划 |
| 55 | Provider 失败时如何降级而非盲目切换 | Provider、异常 | 当前无自动故障转移；架构规划 |
| 56 | 前端和网关如何形成稳定版本化契约 | Java/Gateway、API | 当前映射待验证；架构规划 |

### Level 5：综合架构与面试表达（4 题）

| 题号 | 预留题目主题 | 覆盖知识点 | 标签边界 |
|---:|---|---|---|
| 57 | 多实例下会话、同步任务和数据一致性总方案 | Checkpoint、幂等、一致性 | 现状与规划必须分层 |
| 58 | 从 Provider、Redis、MySQL 到 Agent 的故障演练 | 全链路可靠性 | 现状部分源码确认；结果待验证 |
| 59 | 设计可信推荐闭环并说明安全取舍 | 回查、过滤、脱敏、降级 | 当前缺口；架构规划 |
| 60 | 10 分钟讲清项目亮点、边界和演进路线 | 全部知识树 | Word与源码严格区分 |
...
| 6. 异常和性能 | 全局异常、HTTP status、超时、批量、成本、测试和观测 | 27, 35, 38-44, 49-50, 53, 55, 58 | 能给出失败分类、P95/成本指标和可执行测试，但不编造实测数值 |
...
| 核心知识点 | 基础题 | 原理/场景题 | 深挖/架构题 | 检查结论 |
|---|---|---|---|---|
| 商品加载/同步 | 1,2,12 | 13,35,42 | 45,51,57 | 通过 |
| Chunk/Embedding/Vector | 4,5,6 | 14,16-18,21,32 | 48-50,53,58 | 通过 |
| Agent/Tool/Schema | 8-10 | 20-22,31,33,34 | 46,52,54,59 | 通过 |
| Checkpoint/会话 | 25 | 36,37 | 52,57,58 | 通过 |
| Provider/异常 | 5,11,26-27 | 38,41,43 | 47,49,55,58 | 通过 |
| 前端/Java 集成 | 12,24,28-29 | 36-40,44 | 56-57,60 | 通过 |
| 测试/生产化 | 30 | 41-43 | 45-51,53,57-60 | 通过 |
