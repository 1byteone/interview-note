...
# 预览变更（不写入）
python3 scripts/sync_stars.py --dry-run

# 仅同步指定仓库
python3 scripts/sync_stars.py --repo=owner/name
```

脚本会自动：
1. 调用 GitHub API 获取每个仓库的最新 Stars 数
2. 更新 `guide/guide_repos.json`
3. 更新 `guide/repositories/{name}.md` 的 header 和 Metadata 表
4. 每请求间隔 500ms 避免限流
...
- GitHub REST API 未认证限额 60 次/小时，认证后 5000 次/小时；脚本 22 个请求远低于限额
